            ...
            <div class="container-fluid">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="dataTable">
                        ...
                        <tbody>
                        <tr>
                            <c:forEach var="album" items="${albums}" varStatus="No">
                        <tr>
                            <td>${No.index + 1}</td>
                            <td name = "name" value="${album.name}">${album.name}</td>
                            <td>${album.users.name}</td>
                            <td>${album.status}</td>
                            <td>
                                <form id="deleteForm-${album.id}" action="${pageContext.request.contextPath}/album/ad_deleteAlbum" method="post">
                                    <input type="hidden" name="id" value="${album.id}" />
                                    <button type="submit" onclick="confirmDelete(event, '${album.id}')" class="btn btn-danger">Delete</button>
                                </form>
                            </td>
                            </c:forEach>
                        </tr>
